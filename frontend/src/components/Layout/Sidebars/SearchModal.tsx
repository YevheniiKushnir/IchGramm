...
import { arrowBack, cancel } from "../../../assets/general_icons/index.ts";
...
} from "../../../utils/apiCalls/userApi.ts";
...
const SearchModal: React.FC<SearchModalProps> = ({
  ...
  const searchResults = useSelector(
    (state: RootState) => state.user.search_results
  );
  ...
  const handleSearch = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const filtered = users.filter((user: CondensedUser) =>
  ...
  return (
    <div
      className="absolute z-50 left-0 top-0 lg:-top-7 h-[calc(100vh-81px)] md:h-screen w-screen lg:w-[calc(100vw-244px)]  lg:left-[220px] "
      style={{ backgroundColor: "rgba(0, 0, 0, 0.65)" }}
      onClick={closeModal}
    >
      <div
        className={`bg-decobackground h-[calc(100vh-81px)] md:h-screen md:rounded-r-xl transition-all duration-300 md:py-5 md:px-6 z-50 ${
        ...
        <div className="px-6 md:px-0 py-5 md:py-0">
          ...
          <div className="flex flex-col mt-12 gap-3">
            {matchingUsers?.length > 0
              ? matchingUsers.map((user: CondensedUser) => (
                  <Link
