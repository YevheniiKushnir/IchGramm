...
import { like, comment, liked } from "../../assets/post/index.ts";
import {
  search,
  notifications as notificationsImg,
} from "../../assets/nav_icons/index.ts";
import { done } from "../../assets/bg_deco_imgs/index.ts";
import { logo } from "../../assets/general_icons/index.ts";
import { Post } from "../../store/types/instanceTypes.ts";
import {
  SearchModal,
  NotificationsModal,
} from "../../components/Layout/Sidebars";
import { PhotoCarousel } from "../../components/Posts";
...
const HomePage = () => {
  ...
  if (posts.length === 0 && !isInitialLoading) {
    return (
      <div className="flex flex-col gap-4 justify-center items-center mt-8 mx-4">
        <img src={logo} alt="Ichgram" />
        <p className="text-xl ">Follow users to see their posts here</p>
      </div>
    );
  } else if (posts.length === 0) return <Spinner />;
  ...
  return (
    <div className="flex flex-col justify-center">
      ...
      <div
        className={isNotificationsOpen ? "opacity-100" : "opacity-0 invisible"}
      >
        <NotificationsModal
      ...
      <div
        className="grid grid-cols-1 lg:grid-cols-2 gap-x-10 gap-y-6 my-6 md:my-14 mx-[5vw] sm:mx-auto"
      >
        {posts?.length > 0 &&
          posts.map((post) => (
            <div
            ...
            >
              ...
              {post?.content?.length < 82 ? (
                ...
                <div>
                  <p className="text-xs mb-2 max-w-[420px]">
                    ...
                    <span
                      ...
                      onClick={(e) => {
                        ...
                        const fullText =
                          contentP.nextElementSibling as HTMLElement;
                  ...
                  <p hidden className=" text-xs mb-2 max-w-[420px]">
                    <span className="font-semibold pr-2">
                      {post?.author?.username}
                    </span>
                    {post.content}
                    <span
                      ...
                      onClick={(e) => {
                        ...
                        const contentP = target.parentElement as HTMLElement;
                        const shortenedText =
                          contentP.previousElementSibling as HTMLElement;
                        contentP.hidden = true;
              ...
              <p className="text-darkgray text-xs mb-9">
      ...
      {!hasMore && (
        <div className="flex flex-col items-center mx-auto pb-16">
